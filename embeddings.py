...
class UserDocumentCollection:
    def __init__(self, user_name: str):
        ...
        self.deployment_name="embedding-model"
    ...
    def reset_and_run_indexer(self):
        """
        Force a full re-index of the data source by resetting the indexer's
        checkpoint state and immediately running it.
        """
        try:
            # --- Step 1: Reset checkpoint (forget what was previously indexed)
            self.search_indexer_client.reset_indexer(self.indexer_name)
            logger.info(f"🧹 Indexer '{self.indexer_name}' checkpoint reset successfully.")

            # --- Step 2: Run the indexer (start full crawl now)
            self.search_indexer_client.run_indexer(self.indexer_name)
            logger.info(f"🚀 Indexer '{self.indexer_name}' started successfully.")

        except HttpResponseError as e:
            logger.error(
                f"❌ Failed to reset or run indexer '{self.indexer_name}': {e.message}"
            )
        except Exception as e:
            logger.exception(
                f"❌ Unexpected error while resetting or running indexer '{self.indexer_name}': {str(e)}"
            )
